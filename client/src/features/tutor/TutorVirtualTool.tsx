import { Eraser, FileUp, Paperclip, Play, Presentation, StopCircle, Trash2, Video } from "lucide-react";
...
type Recording = { title: string; length: string; date: string };
  ...
  return (
    <>
      ...
      <section className="mt-7 rounded-[27px] border border-[#e3e8df] bg-white p-6 sm:p-7">
        ...
        {tab === "recordings" && (
          <div className="mt-6">
            <div className="flex items-center justify-between"><div><div className="text-sm font-semibold text-[#25483c]">Session recording</div><p className="mt-1 text-xs text-[#8aa096]">Record with consent, then it lands in the class library for revision.</p></div>{recordingTitle === "active" ? <button onClick={() => { const title = `Session ${new Date().toLocaleDateString(undefined, { month: "short", day: "numeric" })}`; setRecordings((current) => [{ title, length: fmt(recordingLength), date: "Just now" }, ...current]); setRecordingTitle(""); setRecordingLength(0); toast.success("Recording stopped and saved to the library."); }} className="flex items-center gap-2 rounded-full bg-[#fff0ec] px-4 py-2.5 text-xs font-semibold text-[#a25142] hover:bg-[#ffe4dc]"><StopCircle size={14} /> Stop · {fmt(recordingLength)}</button> : <button onClick={() => { setRecordingTitle("active"); toast("Recording in progress — consent banner shown to learners."); }} className="flex items-center gap-2 rounded-full bg-[#173f31] px-4 py-2.5 text-xs font-semibold text-white hover:bg-[#286b51]"><Play size={14} fill="currentColor" /> Start recording</button>}</div>
            <div className="mt-5 space-y-2.5">{recordings.map((recording) => <div key={recording.title} className="flex items-center gap-3 rounded-2xl border border-[#e9eee5] p-3.5"><div className="grid h-9 w-9 place-items-center rounded-xl bg-[#eaf2ff] text-[#3975aa]"><Video size={15} /></div><div className="min-w-0 flex-1"><div className="truncate text-sm font-semibold text-[#25483c]">{recording.title}</div><div className="text-xs text-[#8aa096]">{recording.length} · {recording.date}</div></div><button onClick={() => toast(`Sharing ${recording.title} with the class.`)} className="rounded-full bg-[#f4f7ef] px-3 py-2 text-xs font-semibold text-[#34775e] hover:bg-[#eef4ea]">Share</button><button onClick={() => setRecordings((current) => current.filter((item) => item.title !== recording.title))} className="rounded-full p-2 text-[#c07f73] hover:bg-[#fff0ec]"><Trash2 size={14} /></button></div>)}</div>
          </div>
        )}
      </section>
    </>
