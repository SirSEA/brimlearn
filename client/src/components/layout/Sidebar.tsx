  ...
  return (
    <aside className="flex h-full w-[248px] flex-col bg-[#3B241A] px-5 py-6 text-white">
      <div className="flex items-center justify-between">
        <BrimMark />
        {onClose && <button className="rounded-lg p-2 text-[#E7D8C8] hover:bg-white/10" onClick={onClose}><X size={18} /></button>}
      </div>

      <div className="mt-9 rounded-2xl border border-white/10 bg-white/[0.06] p-2">
        <div className="px-3 pb-2 pt-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#E7D8C8]">Viewing as</div>
      ...
      <nav className="mt-8 space-y-1">
        <div className="mb-3 px-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#E7D8C8]">Workspace</div>
        {tabs.map((item) => {
          ...
          return (
            <button
              ...
              className={`group flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm transition ${selected ? "bg-white/10 text-white" : "text-[#E7D8C8] hover:bg-white/[0.06] hover:text-white"}`}
            >
              <item.icon size={17} className={selected ? "text-[#FFC857]" : "text-[#D9C4B0]"} />
              <span className="flex-1">{item.label}</span>
              {item.modal && <ArrowUpRight size={13} className="text-[#D9C4B0]" />}
      ...
      <div className="mt-auto rounded-2xl border border-white/10 bg-[#3B241A] p-4">
        ...
        <p className="text-xs leading-5 text-[#EDDED0]">Short, steady practice beats cramming. Your best streaks happen before 9am.</p>
      ...
      <div className="mt-5 flex items-center gap-3 border-t border-white/10 pt-4 text-[#E7D8C8]">
        <button className="rounded-lg p-2 text-[#E7D8C8] hover:bg-white/10" onClick={() => setLocation("/settings")}><Settings2 size={16} /></button>
        <button className="rounded-lg p-2 text-[#E7D8C8] hover:bg-white/10" onClick={() => toast("Help center coming soon.")}><LifeBuoy size={16} /></button>
        ...
        <span className="ml-auto text-[10px] text-[#D9C4B0]">v0.1 demo</span>
      </div>
    </aside>
  );
}