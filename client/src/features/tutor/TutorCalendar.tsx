...
const seedEvents: CalendarEvent[] = [
  { id: "e1", dateOffset: 0, time: "03:00 PM", kind: "live", title: "Live class · Multiplication in the market", meta: "JSS1 · BrimLearn Live", tone: "bg-[#eaf2ff] text-[#3975aa]", url: null },
  { id: "e2", dateOffset: 0, time: "06:00 PM", kind: "deadline", title: "Fractions worksheet due", meta: "Whole class · auto-marked", tone: "bg-[#fff1d7] text-[#916d22]", url: null },
  { id: "e3", dateOffset: 1, time: "04:00 PM", kind: "meeting", title: "Parent check-in · Amara Okafor", meta: "Video call · 30 min", tone: "bg-[#e5f5ed] text-[#34775e]", url: null },
  { id: "e4", dateOffset: 2, time: "10:00 AM", kind: "live", title: "Comprehension circle · small group", meta: "JSS1 · BrimLearn Live", tone: "bg-[#eaf2ff] text-[#3975aa]", url: null },
  { id: "e5", dateOffset: 2, time: "01:00 PM", kind: "meeting", title: "Curriculum planning · English dept", meta: "Meeting room · 45 min", tone: "bg-[#e5f5ed] text-[#34775e]", url: null },
  { id: "e6", dateOffset: 4, time: "11:00 AM", kind: "deadline", title: "Weekend practice pack due", meta: "Whole class · 3 subjects", tone: "bg-[#fff1d7] text-[#916d22]", url: null },
  { id: "e7", dateOffset: 5, time: "02:00 PM", kind: "live", title: "States of matter lab demo", meta: "JSS1 · BrimLearn Live", tone: "bg-[#eaf2ff] text-[#3975aa]", url: null },
];
                  ...
                  {dayEvents.map((event) => {
                    ...
                    return (
                      <div key={event.id} className="flex flex-col gap-3 rounded-xl bg-[#fafbf8] p-3 sm:flex-row sm:items-center">
                        ...
                        <div className="flex items-center gap-3"><span className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-[#527064] shadow-sm">{event.time}</span><button onClick={() => joinEvent(event)} className="rounded-full bg-[#173f31] px-3.5 py-2 text-xs font-semibold text-white hover:bg-[#286b51]">{event.kind === "live" ? "Start teaching" : event.kind === "meeting" ? "Open" : "Remind"}</button></div>
                      </div>
                    );
                  })}
                </div>
        ...
        <div className="fixed inset-0 z-[60] grid place-items-center bg-[#0e2b22]/45 p-4" onClick={() => setScheduling(false)}>
          <div className="w-full max-w-lg rounded-[28px] bg-white p-7 shadow-2xl sm:p-8" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="mt-5">
              ...
              {kind === "live" && <div className="mt-3 rounded-2xl bg-[#eaf2ff] p-3 text-[11px] leading-5 text-[#3975aa]">Live sessions run in BrimLearn's embedded classroom. You start it as host; learners join the same room from their Classroom tab — video, chat, and hand-raising included.</div>}
            </div>
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <label className="block text-xs font-semibold text-[#527064]">Details (optional)<span className="mt-1.5 block"><input value={meta} onChange={(event) => setMeta(event.target.value)} placeholder={kind === "live" ? "JSS1 · BrimLearn Live (embedded room)" : kind === "meeting" ? "Video call · 30 min" : "Whole class · auto-marked"} className="w-full rounded-xl border border-[#e1e8df] bg-white px-3 py-2.5 text-xs font-semibold text-[#25483c] outline-none placeholder:font-medium placeholder:text-[#a7b5ad] focus:border-[#5d9c7d]" /></span></label>
            </div>
